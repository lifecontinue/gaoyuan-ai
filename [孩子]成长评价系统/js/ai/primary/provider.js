/* =========================================================
   ai/primary/provider.js — 小学阶段 Provider
   结构同 kindergarten/provider.js，共享 memory 与 schema，独立 prompt
   ========================================================= */
...
async function call(messages) { return doChat(false, messages); }
async function stream(messages, onToken) { return doChat(true, messages, onToken); }

async function doChat(streamMode, messages, onToken) {
  const payload = {
    model: MODEL, messages: Array.isArray(messages) ? messages : [],
    temperature: 0.6, max_tokens: 2000, stream: !!streamMode,
  };
  // 1. 代理优先
  ...
  if (proxyBase) {
    try {
      ...
      if (res.ok) {
        if (!streamMode) { const j = await res.json(); return (j.choices?.[0]?.message?.content) || ''; }
        await readSSE(res, onToken); return;
      }
    } catch (e) { /* 直连 */ }
  }
  // 2. 直连 DeepSeek
  ...
  if (!res.ok) {
    const t = await res.text(); let m = 'AI 服务暂不可用';
    try { m = JSON.parse(t).error?.message || m; } catch (e) { /**/ }
    throw new Error(m);
  }
  if (!streamMode) { const j = await res.json(); return (j.choices?.[0]?.message?.content) || ''; }
...
async function readSSE(res, onToken) {
  ...
  try { while (true) { const { done, value } = await reader.read(); if (done) break; buf += decoder.decode(value, { stream: true }); const lines = buf.split('\n'); buf = lines.pop(); for (const line of lines) { const t = line.trim(); if (!t.startsWith('data:')) continue; const d = t.slice(5).trim(); if (d === '[DONE]') return; try { const j = JSON.parse(d); const c = j.choices?.[0]?.delta?.content; if (c) onToken(c); } catch (e) { /**/ } } } } finally { try { reader.cancel(); } catch (e) { /**/ } }
}
...
export const PrimaryProvider = {
  ...
  async welcomeStream(onToken) {
    ...
    const msgs = [sys, { role: 'user', content: '请用3–4句话向家长自我介绍：你是小学学科评估专家，能结合2022课标九大学科与学科核心素养分析孩子表现并给出学习建议；简要说明当前孩子的数据概览（若有），以及你可以帮家长做什么。输出为 JSON。' }];
  ...
  async chatStream(text, focus, snapshot, onToken) {
    ...
    if (focus) {
      if (focus.domain) userParts.push('当前聚焦学科/领域：' + focus.domain);
  ...
  async actionStream(actionId, focus, snapshot, onToken) {
    const instructions = {
      trend: '请基于提供的成长数据快照，分析孩子最近各学科的发展趋势，指出值得关注的信号。',
      weak: '请基于数据指出当前相对薄弱的学科/素养维度，给出 2–3 条具体可操作的家庭辅导与学习建议。',
      plan: '请基于薄弱点与优势，为孩子制定可落地的学科提升计划，包含每周可执行的小任务。',
      report: '请生成一份面向家长的简短学科成长综述。',
      activity: '请推荐 2–3 个结合生活的学习游戏/亲子活动，说明对应维度、怎么玩、观察什么。如需要也可以搜索当前适合小学的学习资源和活动推荐。',
      subjects: '请基于当前小学学科维度数据，分析孩子三年级各学科发展情况，给出具体建议。',
    };
    const sys = await this.buildSystem();
    ...
    if (actionId === 'activity') {
      const searchData = await webSearch('小学生 学习活动 游戏 推荐 ' + new Date().getFullYear(), 3);
      if (searchData) {
        const ctx = searchContextBlock(searchData);
        if (ctx) userParts.push(ctx);
      }
